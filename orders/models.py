...
class ComboOrderItem(models.Model):    
    ...
    def __str__(self):
        return f"{self.combo.name} ({self.subscription_plan}) for Order #{self.order.id}"
...
class Cart(models.Model):
    ...
    def get_subtotal(self):
        meal_subtotal = sum(item.get_total_price() for item in self.cart_items.all())
        combo_subtotal = sum(item.get_total_price() for item in self.combo_cart_items.all())
        return meal_subtotal + combo_subtotal
    ...
    def get_items_count(self):
        return self.cart_items.count() + self.combo_cart_items.count()

    def clear(self):
        self.cart_items.all().delete()
        self.combo_cart_items.all().delete()
...
class CartItem(models.Model):
    ...
    def __str__(self):
        if self.custom_meal:
            return f"CustomMeal ({self.custom_meal.category}) x {self.quantity}"
        elif self.meals:
...
class ComboCartItem(models.Model):
    ...
    cart = models.ForeignKey(Cart, related_name='combo_cart_items', on_delete=models.CASCADE)
    combo = models.ForeignKey(Combo, on_delete=models.CASCADE)
    quantity = models.PositiveIntegerField(default=1)
    subscription_plan = models.CharField(max_length=20, choices=SUBSCRIPTION_PLAN)
    delivery_from_date = models.DateField(null=True, blank=True)
    delivery_to_date = models.DateField(null=True, blank=True)
    delivery_time_slot = models.CharField(max_length=50,choices=DELIVERY_TIME_SLOTS,null=True,blank=True)
    delivery_time = models.DateTimeField(null=True, blank=True)
    preferences = models.TextField(blank=True)
    added_at = models.DateTimeField(auto_now_add=True)
    ...
    class Meta:
        ordering = ['-added_at']
        unique_together = ['cart', 'combo', 'subscription_plan']
    
    def get_price_per_item(self):
        return self.combo.get_total_price()
    ...
    def __str__(self):
        return f"{self.combo.name} ({self.subscription_plan}) x {self.quantity}"